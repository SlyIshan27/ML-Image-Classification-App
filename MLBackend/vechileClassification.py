    ...
    for imageRoot, imageDirectories, imageFiles in os.walk(directory):
        for image in imageFiles:
            ...
            try:
                ...
                # os.remove(imagePath)
def loadData():
    data = tf.keras.utils.image_dataset_from_directory('vechileD/Vehicles')
    ...
    return trainDataset, valDataset, testDataset, classNames
...
if __name__ == "__main__":
    train, val, test, classNames = loadData()
    print(classNames)
    # print(train)
    model = buildModel()
    hist = trainModel(train, val, model)
    model.save(os.path.join('models', 'vechileClassificationV2.h5'))
    plotLoss(hist)
    plotAccuracy(hist)
